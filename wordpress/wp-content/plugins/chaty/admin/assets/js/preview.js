...
jQuery(document).ready(
    function () {
        ...
        jQuery(document).on(
            "mouseover",
            ".csaas.open-on-hover .csaas-i-trigger:not(.single-channel)",
            function () {
        ...
        jQuery(document).on(
            "click",
            ".csaas-i-trigger:not(.single-channel)",
            function () {
        ...
        jQuery(document).on(
            "click",
            ".csaas-close-button, .csaas-close-agent-list",
            function (e) {
                e.preventDefault();
                e.stopPropagation();
                jQuery(".csaas-outer-forms").removeClass("active");
        ...
        jQuery(document).on(
            "keyup",
            "#cht_social_message_Contact_Us_form_title",
            function () {
        ...
        jQuery(document).on(
            "change",
            ".form-field-setting-col input[type='text']",
            function () {
                change_custom_preview();
            }
        );
        ...
        jQuery(document).on(
            "click",
            ".form-field-setting-col input[type='checkbox']",
            function () {
                change_custom_preview();
            }
        );
        ...
        jQuery(document).on(
            "change",
            ".chaty-agent-name, #chaty_default_state, input[name='chaty_icons_view']:checked",
            function () {
                change_custom_preview();
            }
        );
        ...
        jQuery(document).on(
            "keyup",
            "input[name='cht_close_button_text']",
            function () {
                change_custom_preview();
            }
        );
        ...
        jQuery(document).on(
            "click",
            ".chaty-preview input, .chaty-preview button",
            function (e) {
                e.preventDefault();
            }
        );
        ...
        jQuery(document).on(
            "click",
            ".csaas-channel.csaas-agent-button",
            function (e) {
                e.preventDefault();
                // e.stopPropagation();
                var dataForm = jQuery(this).data('form');
        ...
        jQuery(document).on(
            "click",
            "#trigger_on_time, #chaty_trigger_on_scroll, #cht_close_button",
            function () {
                change_custom_preview();
            }
        );
...
function change_custom_preview()
{
    ...
    if (jQuery("#channels-selected-list > li").length > 0) {
        ...
        if (activeChannels) {
            ...
            var widgetHtml      = "<div style='display:none' class='csaas csaas-id-0 csaas-widget-0 csaas-key-0' id='csaas-widget-0' data-key='0' data-id='0' data-identifier='0' data-nonce='0' >" + "<div class='csaas-widget " + widgetPosition + "-position'>" + "<div class='csaas-channels'>" + "<div class='csaas-channel-list'></div>" + "<div class='csaas-i-trigger'></div>" + "</div>" + "</div>" + "</div>";
            ...
            if (previewChannelList.length == 1) {
                ...
                var widgetButton = '<div class="csaas-channel csaas-cta-main has-on-hover csaas-tooltip ' + ctaToolTipPosition + ' active" data-widget="0" >' + '<span class="on-hover-text">' + ctaText + '</span>' + '<div class="csaas-cta-button">' + '<button type="button" class="open-csaas">' + widgetIcon + '</button>' + '<button type="button" class="open-csaas-channel"></button>' + '</div>' + '</div>';
                jQuery("#csaas-widget-0 .csaas-i-trigger").html(widgetButton);

                /* close button */
                var closeHtml = '<div class="csaas-channel csaas-cta-close csaas-tooltip ' + toolTipPosition + '" data-hover="' + jQuery("input[name='cht_close_button_text']").val() + '">' + '<div class="csaas-cta-button"><button type="button">' + '<span class="csaas-svg">' + '<svg viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg"><ellipse cx="26" cy="26" rx="26" ry="26" fill="' + jQuery("input[name='cht_color']:checked").val() + '"></ellipse><rect width="27.1433" height="3.89857" rx="1.94928" transform="translate(18.35 15.6599) scale(0.998038 1.00196) rotate(45)" fill="white"></rect><rect width="27.1433" height="3.89857" rx="1.94928" transform="translate(37.5056 18.422) scale(0.998038 1.00196) rotate(135)" fill="white"></rect></svg>' + '</span>' + '</button>' + '</div>' + '</div>';
            ...
            advanceCustomCSS += "#csaas-widget-0 .csaas-channel .custom-csaas-image {width: " + widgetSize + "px; height: " + widgetSize + "px; }";
            ...
            advanceCustomCSS += "#csaas-widget-0 .csaas-channel .csaas-custom-icon {display:block; width: " + widgetSize + "px; height: " + widgetSize + "px; line-height: " + widgetSize + "px; font-size: " + parseInt(widgetSize / 2) + "px; }";
    ...
    var font_val = jQuery("select.form-fonts").val();
    jQuery('#chaty-google-font').remove();
    if (font_val != "") {
        if(jQuery('select.form-fonts option:selected').data("group") != "Default") {
            jQuery('head').append('<link href="https://fonts.googleapis.com/css?family=' + font_val + ':400,600,700" rel="stylesheet" type="text/css" id="chaty-google-font">');
        }
...
function createAgentList(channel, widgetId)
{
    ...
    if (jQuery("#chaty-social-" + channel + " .chaty-agent-list .agent-list .agent-channel-setting").length) {
        jQuery("#chaty-social-" + channel + " .chaty-agent-list .agent-list .agent-channel-setting").each(
            function () {
...
function startMakingContactForm(channel, widgetId)
{
    ...
    formHtml += "</div>";
    // csaas-contact-inputs
    formHtml += "</div>";
    // csaas-contact-form-body
    // formHtml += "</form>";
    formHtml += "</div>";
    formHtml += "</div>";
    formHtml += "</div>";
...
function getActiveChannels()
{
    ...
    if (jQuery("#channels-selected-list > li:not(.chaty-cls-setting)").length) {
        var isDesktop = jQuery("#previewDesktop").is(":checked") ? true : false;
        jQuery("#channels-selected-list > li:not(.csaas-cls-setting)").each(
            function () {
